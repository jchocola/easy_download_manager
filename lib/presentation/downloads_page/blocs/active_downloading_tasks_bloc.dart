...
class ActiveDownloadingTasksBloc
  ...
  void _onLoad(ActiveDownloadingTasksEvent_Load event,
      Emitter<ActiveDownloadingTasksState> emit) async {
    ...
    try {
      ...
      emit(ActiveDownloadingTasksState_Error(
          message: 'Failed to load active downloads: $e'));
    }
  }

  void _onRefresh(ActiveDownloadingTasksEvent_Refresh event,
      Emitter<ActiveDownloadingTasksState> emit) async {
    try {
      final tasks = await _repository.getAllDownloadingTasks();
      // Filter only active tasks (not completed, failed, or canceled)
