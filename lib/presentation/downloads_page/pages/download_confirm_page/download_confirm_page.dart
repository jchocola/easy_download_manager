  ...
  Widget build(BuildContext context) {
    ...
    return BlocListener<AddDownloadBloc, AddDownloadBlocState>(
      listener: (context, state) {
        if (state is AddDownloadBlocStateSucces) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text(l10n.downloadAdded)),
          );
          context.go('/downloads');
        } else if (state is AddDownloadBlocStateError) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text(l10n.downloadFailedToAdd)),
          );
        }
      },
      child: Scaffold(
          ...
          children: [
            ...
            BlocBuilder<AddDownloadBloc, AddDownloadBlocState>(
              builder: (context, state) {
                final isLoading = state is AddDownloadBlocStateLoading;
                return Column(
                  children: [
                    BigButton(
                      withGradient: true,
                      ...
                      onTap: isLoading
                          ? null
                          : () => context
                              .read<AddDownloadBloc>()
                              .add(AddDownloadBlocEvent_Submit()),
                    ),
                    if (isLoading)
                      Padding(
